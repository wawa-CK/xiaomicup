...
bool MotionHandler::Init()
{
  ...
  voice_sub_ = node_ptr_->create_subscription<std_msgs::msg::Bool>(
    "audio_notification_report", rclcpp::SystemDefaultsQoS(),
    std::bind(&MotionHandler::VoiceCallback, this, std::placeholders::_1));
...
void MotionHandler::HandleResultCmd(const CmdRequestT request, CmdResponseT response)
{
  ...
  //   action_ptr_->ShowStandElecSkin();
  // }
  if (request->motion_id == 121 || request->motion_id == 172) {
...
void MotionHandler::VoiceCallback(const std_msgs::msg::Bool::SharedPtr msg)
{
  is_voice_playing_ = msg->data;
}
