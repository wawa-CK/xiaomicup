...
public:
  ...
  void VoiceCallback(const std_msgs::msg::Bool::SharedPtr msg);
...
private:
  ...
  bool VoicePlay()
  {
    ...
    request->is_online = false;
    request->speech.play_id = 40002;
    auto future = audio_play_->async_send_request(request);
    auto base_time = std::chrono::system_clock::now();
    auto task_time = base_time + std::chrono::seconds(3);
    while (std::chrono::system_clock::now() <= task_time) {
      if (!is_voice_playing_) {
        is_voice_playing_ = true;
        INFO("Interrupt audio play");
        return true;
      }
    ...
    if (result->status == 0) {
      INFO("Play audio success");
      return true;
    }
    INFO("Play audio failed");
    return false;
  }
  ...
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr voice_sub_ {nullptr};
